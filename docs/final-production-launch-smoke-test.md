...
| Gallery                   | Deployment recheck required                        | Public gallery now uses an image-first dark layout with category filters, nine-image incremental rendering, keyboard-visible overlays, and an accessible click/tap lightbox. Recheck desktop, keyboard, and 390 px mobile behavior after deployment; confirm Meal Plans is the only current meal-plan category. Admin CRUD and image sources are unchanged.                                                                                    |
